...
There are 11 implemented checks and 20 indicators with real, passing, hash-verified automated
...
FedRAMP/rules + FedRAMP/schemas        pinned by sha256, drift-checked
  ...
  catalog (46 indicators, 10 themes, 209 controls, 195 definitions)
        ...
        │
        ▼
  collectors ── AWS · GitHub · pipeline ──► evidence bundles (hash-verified)
...
So every bundle declares `expected`, `examined` and `source_of_truth`, and three invariants are
enforced centrally rather than per collector:
...
3. **An unexplained population gap throws.** `examined ≠ expected` requires a `reconciliation`
   string, because "three accounts denied the call" is a permissions finding while "two roles are
   service-linked and cannot carry a policy" is a scoping decision — and both are invisible in a
   boolean.

Bundles are content-hashed over canonical JSON, so an unchanged control produces no diff and a
changed one produces exactly one. Details in
...
ksi emit sdr --overview-uri URI          # sdr | ocr | oscal-ar
...
- **No real evidence is committed.** A bundle names accounts, roles, buckets and failing resources —
  an accurate inventory of where a boundary is weakest. `.evidence/` is gitignored, and in a real
  deployment the locker belongs in a private append-only store.
